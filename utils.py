...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if args.zca:
        ...
        del images, labels, zca_images, zca
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True, seeds=None):
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        # if datum[1] is (n,n) tensor and type is torch.int
        if len(datum[1].shape) > 1 and datum[1].dtype not in (torch.int16, torch.int32, torch.int64):
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False):
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        ...
        # eval each Epoch/4 times
        # if ep % (Epoch//4) == 0 and ep != 0:
        #     with torch.no_grad():
        #         loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug=False)
        #         acc_eval_list[ep] = acc_test
    ...
    # print eval acc list
    # print("Eval acc list: ", acc_eval_list)

    if return_loss:
        return net, acc_train_list, acc_test, loss_train_list, loss_test #, acc_eval_list
    else:
